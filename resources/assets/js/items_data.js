var ComponentsFormTools = function () {
    ...
    var handleTwitterTypeahead = function() {
		// 料品名稱
        /* var item_name = new Bloodhound({
          datumTokenizer: function(d) { return Bloodhound.tokenizers.whitespace(d.name); },
          queryTokenizer: Bloodhound.tokenizers.whitespace,
          limit: 20,
          prefetch: {
            //url: 'demo/typeahead_countries.json',
			url: 'logs/items_data/item_name.inc',
            filter: function(list) {
              return $.map(list, function(item_name) { return { name: item_name }; });
            }
          }
        });
        ...
        if (Metronic.isRTL()) {
          $('#sel_name').attr("dir", "rtl");  
        } 
        $('#sel_name').typeahead(null, {
          name: 'sel_name',
          displayKey: 'name',
          hint: (Metronic.isRTL() ? false : true),
          source: item_name.ttAdapter()
        }); */
		
        // 型號
        var item_model = new Bloodhound({
          datumTokenizer: function(d) { return Bloodhound.tokenizers.whitespace(d.name); },
          queryTokenizer: Bloodhound.tokenizers.whitespace,
          limit: 10,
          prefetch: {
            //url: 'demo/typeahead_countries.json',
			url: 'logs/items_data/item_model.inc',
            filter: function(list) {
              return $.map(list, function(item_model) { return { name: item_model }; });
            }
          }
        });
 
        item_model.initialize();
         
        if (Metronic.isRTL()) {
          $('#sel_model').attr("dir", "rtl");  
        } 
        $('#sel_model').typeahead(null, {
          name: 'sel_model',
          displayKey: 'name',
          hint: (Metronic.isRTL() ? false : true),
          source: item_model.ttAdapter()
        });
		
		// 顏色
        var item_color = new Bloodhound({
          datumTokenizer: function(d) { return Bloodhound.tokenizers.whitespace(d.name); },
          queryTokenizer: Bloodhound.tokenizers.whitespace,
          limit: 10,
          prefetch: {
            //url: 'demo/typeahead_countries.json',
			url: 'logs/items_data/item_color.inc',
            filter: function(list) {
              return $.map(list, function(item_model) { return { name: item_model }; });
            }
          }
        ...
        item_color.initialize();
         
        if (Metronic.isRTL()) {
          $('#sel_color').attr("dir", "rtl");  
        } 
        $('#sel_color').typeahead(null, {
          name: 'sel_color',
          displayKey: 'name',
          hint: (Metronic.isRTL() ? false : true),
          source: item_color.ttAdapter()
        });
		...
		// 國際條碼
        var item_isbn = new Bloodhound({
          datumTokenizer: function(d) { return Bloodhound.tokenizers.whitespace(d.name); },
          queryTokenizer: Bloodhound.tokenizers.whitespace,
          limit: 10,
          prefetch: {
            //url: 'demo/typeahead_countries.json',
			url: 'logs/items_data/item_isbn.inc',
            filter: function(list) {
        ...
        item_isbn.initialize();
         
        if (Metronic.isRTL()) {
          $('#sel_isbn').attr("dir", "rtl");  
        } 
        $('#sel_isbn').typeahead(null, {
          name: 'sel_isbn',
          displayKey: 'name',
          hint: (Metronic.isRTL() ? false : true),
          source: item_isbn.ttAdapter()
        });
    }
